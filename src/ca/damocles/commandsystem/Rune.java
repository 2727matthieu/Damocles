...
import ca.damocles.itemblueprints.Protection;
import ca.damocles.itemblueprints.SlotRune;
...
public class Rune implements CommandExecutor{
	...
	ca.damocles.enchantsystem.Rune rune = new ca.damocles.enchantsystem.Rune();
	Plugin plugin;
			...
			for(Enchantments enchant : Enchantments.values()){
				if(args[0].equalsIgnoreCase(enchant.toString())){
					try{
						...
						player.getInventory().addItem(new ca.damocles.itemblueprints.Rune(enchant, level, success, destroy).getItemStack());
						return true;
